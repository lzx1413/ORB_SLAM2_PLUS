...
inline cv::Mat PlotBbox(cv::Mat img, NDArray bboxes, NDArray scores, NDArray labels,NDArray masks,
    ...
    for (int i = 0; i < num; ++i) {
        ...
        // draw bounding box
        int left = bboxes.At(0,i,0);
        int top = bboxes.At(0,i,1);
        int right = bboxes.At(0,i,2);
        int bottom = bboxes.At(0,i,3);
        cv::Mat mask_mat= cv::Mat(14,14,CV_32F, const_cast<float *>(mask_data+14*14*i));
        cv::Mat maskb;
        ...
        cv::Rect box = cv::Rect(pt1,pt2);
        cv::resize(mask_mat, mask_mat, cv::Size(box_width, box_height));
//mask_mat = mask_mat(cv::Rect(box.x-left,box.y-top,box.width,box.height));
        cv::inRange(mask_mat,  0.5, 100, maskb);
        cv::Mat roi_img = img(box);
        cv::Mat mask;
        cv::cvtColor(maskb,mask, CV_GRAY2BGR);
        ...
        txt += " " + ss.str();
        //cv::putText(img, txt, cv::Point(pt1.x, pt1.y - 5), 1, 0.6, color, 1);
